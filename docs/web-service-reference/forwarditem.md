...
Das **ForwardItem** -Element enthält ein Exchange-Informationsspeicher Element, das an Empfänger weitergeleitet werden soll. 
...
Das [from](from.md) -Element sollte auf die e-Mail-Adresse des Prinzipals festgelegt werden, wenn ein Element von einem Delegaten weitergeleitet wird. Wenn die Stellvertretung die [from](from.md) -Eigenschaft nicht festgelegt hat, wird das Element scheinbar direkt aus dem Postfach des Stellvertreters gesendet. 
