...
1. <span data-ttu-id="7fd68-p106">In der **configurationNamingContext** -Eigenschaft des DSE-Stammeintrags in AD DS finden Sie den Pfad zum Konfigurationsbenennungskontext für die Domäne. Zum Lesen dieser Eigenschaft können Sie die [DirectoryEntry](http://msdn2.microsoft.com/de-de/library/z9cddzaa)-Klasse oder jede andere API verwenden, die auf AD DS zugreifen kann.</span><span class="sxs-lookup"><span data-stu-id="7fd68-p106">Read the **configurationNamingContext** property of the root DSE entry in AD DS to get the path to the configuration naming context for the domain. You can do this by using the [DirectoryEntry](http://msdn2.microsoft.com/de-de/library/z9cddzaa) class, or any other API that can acces AD DS.</span></span> 
